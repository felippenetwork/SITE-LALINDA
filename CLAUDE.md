...
- `src/integrations/supabase/auth-middleware.ts` (`requireSupabaseAuth`) — validates the caller's Bearer JWT server-side via `supabase.auth.getClaims()`, then hands the handler a *user-scoped* Supabase client (RLS applies) plus `userId`/`claims` in context. Use this middleware, not the admin client, for any mutation gated by user identity.
