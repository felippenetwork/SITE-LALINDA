  ...
  return (
    <div className="min-h-screen bg-background font-serif text-foreground selection:bg-primary selection:text-white">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 pt-28 md:pt-40 pb-24">
        <Link
          href="/produtos"
          className="inline-flex items-center gap-4 text-muted-foreground font-sans uppercase tracking-widest text-[8px] md:text-[9px] font-black mb-8 md:mb-12 hover:text-foreground transition-colors group"
        >
          <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Voltar
          para Produtos
        </Link>

        <div className="bg-white rounded-[3rem] md:rounded-[5rem] shadow-sm overflow-hidden flex flex-col lg:flex-row border border-border">
          <div className="lg:w-1/2 relative aspect-[4/3] sm:aspect-[4/5] lg:aspect-auto group overflow-hidden">
            ...
            {!product.available && (
              <div className="absolute inset-0 bg-foreground/40 backdrop-blur-[2px] flex items-center justify-center">
                <span className="bg-white text-foreground text-[10px] font-sans font-black uppercase tracking-widest px-6 py-3 rounded-full shadow-2xl">
          ...
          <div className="lg:w-1/2 p-8 md:p-16 lg:p-28 flex flex-col justify-center relative">
            <div className="absolute top-0 right-0 w-32 h-32 bg-background rounded-bl-full -z-10 opacity-50"></div>
            <Badge className="w-fit mb-6 md:mb-8 bg-background text-primary hover:bg-accent border-none font-sans uppercase tracking-widest text-[8px] md:text-[9px] px-4 md:px-6 py-2 rounded-full">
              {product.category}
            </Badge>

            <h2 className="text-5xl md:text-7xl lg:text-9xl font-serif italic text-foreground mb-6 md:mb-8 leading-[0.85] tracking-tighter">
              {product.name}
            </h2>

            <p className="text-muted-foreground font-sans leading-relaxed text-base md:text-lg mb-8 md:mb-12 italic">
              {product.description ||
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-10 md:mb-16">
              <div className="p-6 md:p-8 bg-background rounded-[1.5rem] md:rounded-[2rem] border border-border group">
                <div className="flex items-center gap-4 text-primary mb-3 md:mb-4">
                  <Weight className="w-5 h-5 md:w-6 md:h-6" strokeWidth={1.5} />
                  <span className="text-[8px] md:text-[9px] font-sans uppercase tracking-widest font-black text-muted-foreground">
                    Peso Unitário
                  </span>
                </div>
                <span className="text-2xl md:text-3xl font-serif italic text-foreground">
                  {product.weight}
                </span>
              </div>
              <div className="p-6 md:p-8 bg-background rounded-[1.5rem] md:rounded-[2rem] border border-border group">
