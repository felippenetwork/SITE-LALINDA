...
import { getPedidoParaVendedor } from "@/lib/data/vendas";
...
// Espelha app/portal/(loja)/pedidos/[id]/page.tsx, mas lendo via
// getPedidoParaVendedor (service role + has_role) em vez de RLS — a
// policy "Cliente le os proprios pedidos" nunca autorizaria a sessão do
// vendedor a ver o pedido de um cliente que não é ele mesmo.
export default async function VendasPedidoConfirmadoPage({
  params,
}: {
  params: Promise<{ clienteId: string; pedidoId: string }>;
}) {
  const { clienteId, pedidoId } = await params;

  const pedido = await getPedidoParaVendedor(pedidoId);
  if (!pedido) notFound();
