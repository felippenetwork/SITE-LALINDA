...
// Simple string hash mapped into [min, max) — used for pseudo-randomness
// derived from a stable id instead of Math.random() (impure during render).
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Deterministic pseudo-random width between 50 and 90%, derived from a
  // stable per-instance id instead of Math.random() (impure during render).
  const id = React.useId();
  const width = React.useMemo(() => `${hashToRange(id, 50, 90)}%`, [id]);
