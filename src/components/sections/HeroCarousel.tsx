  ...
  return (
    <section className="relative h-[65vh] md:h-[75vh] lg:h-[85vh] overflow-hidden bg-stone-900">
            ...
            <div key={i} className="flex-[0_0_100%] relative h-full overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center px-6 md:px-24">
                <div className="max-w-4xl">
                  ...
                  >
                    <Link to="/produtos" className="inline-flex items-center gap-4 text-white font-sans uppercase tracking-widest text-[10px] md:text-xs font-bold border-b border-white/30 pb-2 hover:border-primary hover:text-primary transition-all">
      ...
      <div className="absolute bottom-6 right-6 md:bottom-12 md:right-12 flex items-center gap-4 md:gap-6 z-20">
        <button onClick={() => emblaApi?.scrollPrev()} className="text-white/50 hover:text-white transition-colors">
          <ChevronLeft className="w-6 h-6 md:w-8 md:h-8" strokeWidth={1} />
        </button>
        <div className="text-white font-serif italic text-xl md:text-2xl">
          0{selectedIndex + 1} <span className="text-white/30 text-xs md:text-sm not-italic font-sans mx-1 md:mx-2">/</span> 0{slides.length}
        </div>
        <button onClick={() => emblaApi?.scrollNext()} className="text-white/50 hover:text-white transition-colors">
